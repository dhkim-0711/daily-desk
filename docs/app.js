...
function weeklyAnalysisSectionBody(section) {
  if (section.title === "핵심 신호") {
    return `
      <div class="weekly-signal-list">
  ...
  $("#weeklyAnalysisBody").innerHTML = `
    ...
    <div class="weekly-analysis-grid weekly-modal-grid">
      ${sections.map((section) => `
        <section class="${section.title === "핵심 신호" ? "weekly-main-section" : ""}">
...
function weeklyArticleAnalysis(article) {
  ...
  let impact = `이 이슈의 산업적 파급은 ${companyText}를 둘러싼 단기 뉴스에 그치지 않고, AI 반도체 기업이 어떤 고객 문제를 실제로 해결할 수 있는지 검증하는 압력으로 이어집니다. 기술 성능, 가격, 전력 효율, 공급 안정성, 소프트웨어 지원이 한꺼번에 평가되기 때문에 국내 NPU 기업에는 제품 스펙보다 실제 도입 사례와 운영 데이터가 더 중요한 경쟁 자산이 됩니다.`;
  ...
  let followUp = `${companyText}가 실제 고객, 매출, 제품 적용 사례로 이어지는지와 ${tagText} 관련 신호가 다른 기사에서도 반복되는지 확인해야 합니다.`;

  if (hasSupplyChain) {
    core = `"${title}"은 AI칩 경쟁이 설계 성능을 넘어 파운드리, 패키징, 고객사 확보 등 공급망 실행력으로 확장되고 있음을 보여줍니다.`;
    impact = `이 이슈는 AI칩 경쟁이 설계사 단독의 성능 경쟁에서 제조, 패키징, 검증, 고객 납기를 묶은 밸류체인 경쟁으로 넓어지고 있음을 뜻합니다. ${companyText}처럼 공급망 우선순위를 확보하는 플레이어가 늘수록 국내 NPU 기업은 타깃 공정 선택, 초도 물량 확보, 패키징 파트너십, 수요처와의 일정 관리 역량까지 함께 증명해야 합니다. 산업적으로는 설계 IP의 우수성보다 양산 가능성과 납기 신뢰성이 투자·조달 판단의 핵심 지표로 올라옵니다.`;
    ...
    followUp = "후속으로는 실제 양산 일정, 파운드리·패키징 파트너 공개 여부, 수요기업의 구매 확약 또는 PoC 전환 여부를 봐야 합니다. 정책적으로는 MPW, 검증 바우처, 수요처 공동 실증을 묶을 수 있는지 확인할 필요가 있습니다.";
  } else if (hasInfrastructure) {
    core = `"${title}"은 AI 인프라 투자가 데이터센터, GPU 병목, 전력 비용, 클라우드 운용비 문제와 함께 움직이고 있음을 보여줍니다.`;
    impact = `데이터센터와 클라우드 비용 이슈가 커질수록 GPU를 얼마나 많이 확보하느냐가 곧 시장 진입장벽이 됩니다. 이때 국산 NPU는 범용 GPU를 전면 대체하기보다 반복 추론, 특정 모델 서빙, 온프레미스·공공 AI 서비스처럼 전력비와 운용비가 민감한 영역에서 실질적인 대안이 될 수 있습니다. 산업적으로는 칩 기업 혼자보다 IDC, 클라우드, SI, 모델 최적화 기업이 함께 움직이는 패키지형 생태계가 중요해집니다.`;
    ...
    followUp = "관찰할 후속 이슈는 데이터센터 투자 규모, 전력비·운영비 절감 수치, 클라우드 사업자의 NPU 적용 여부입니다. 정책적으로는 공공 데이터센터 실증, 전력 대비 성능 벤치마크, 추론 서비스 조달 모델과 연결할 수 있습니다.";
  } else if (hasDomesticNpu) {
    core = `"${title}"은 국내 NPU 기업이 연구개발 단계를 지나 실제 고객, 자금, 양산, 상장, 서비스 적용과 연결되는 사업화 국면에 들어섰다는 신호입니다.`;
    impact = `${companyText} 관련 이슈는 국내 NPU 기업이 시제품과 기술 홍보 단계를 넘어 고객 검증, 매출 전환, 양산 준비, 서비스 적용 사례를 요구받는 단계로 이동했음을 보여줍니다. 이 국면에서는 개별 칩의 TOPS 수치보다 개발도구, SDK, 모델 포팅, 서버·모듈 파트너, 유지보수 체계가 함께 평가됩니다. 산업적으로는 한두 기업의 성과가 국내 AI 반도체 생태계 전체의 신뢰도로 번질 수 있어, 성공 사례를 표준 레퍼런스로 만드는 작업이 중요합니다.`;
    ...
    followUp = "후속으로는 고객사 공개, 양산·상장 일정, 매출 전환 가능성, SDK·개발도구 공개 수준을 확인해야 합니다. 정책 연결점은 K-엔비디아 참여기업별 수요처 매칭, 공공 PoC, 해외 고객 검증 트랙입니다.";
  } else if (hasPolicy) {
    core = `"${title}"은 정부·공공기관의 AI 정책과 사업이 민간 수요 창출, 인프라 확보, 생태계 조성으로 연결되는 흐름을 보여줍니다.`;
    impact = `정책·공공 이슈의 파급효과는 예산 규모 자체보다 시장에 어떤 수요 신호를 주느냐에서 갈립니다. 공공사업이 성능평가, 조달, 실증, 데이터센터 적용까지 이어지면 국내 NPU 기업은 민간 고객에게 보여줄 신뢰도 높은 레퍼런스를 확보할 수 있습니다. 반대로 공모·행사 중심으로 끝나면 산업에는 홍보 효과만 남기 쉽기 때문에, 정책 사업은 기업 지원보다 구매자와 운용 현장을 만드는 방향으로 설계되어야 합니다.`;
    ...
    followUp = "확인할 후속 이슈는 실제 공고 여부, 예산 규모, 지원 대상, 성과지표가 실증·조달·수요처 확보로 설계되는지입니다. 정책적으로는 공공 레퍼런스와 민간 수요 전환 장치가 포함되는지 봐야 합니다.";
  } else if (hasMarket) {
    core = `"${title}"은 AI 시장의 수요, 투자, 실적 기대가 반도체와 인프라 기업의 가치평가에 직접 반영되고 있음을 보여줍니다.`;
    impact = `시장 이슈는 AI 반도체 기업의 자금조달 비용, 고객의 구매 타이밍, 투자자의 평가 기준을 동시에 흔듭니다. ${companyText}와 연결된 실적·투자·수요 뉴스가 강해질수록 국내 NPU 기업도 기술 가능성보다 매출 전환성, 고객군의 질, 단가 경쟁력, 양산 일정의 현실성을 더 엄격하게 검증받게 됩니다. 산업적으로는 거대 GPU 생태계와 정면 경쟁하기보다 금융, 제조, 공공, 엣지 추론처럼 비용 절감 효과를 수치로 보여줄 수 있는 시장부터 열 가능성이 큽니다.`;
    ...
    followUp = "후속으로는 CAPEX 변화, 고객 계약, 투자 유치, 주가 반응, AI 서비스 매출 전환 여부를 확인해야 합니다. 정책 연결점은 투자 연계 IR, 수요기업 바우처, 성능·비용 절감 지표 표준화입니다.";
  }
