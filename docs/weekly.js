...
function renderWeeklyMarkdown(markdown = "") {
  ...
  const closeQuote = () => {
    if (!inQuote) return;
    html.push(`<blockquote>${quoteLines.map(inlineWeeklyMarkdown).join("<br>")}</blockquote>`);
    inQuote = false;
    quoteLines = [];
  };
  ...
  for (const rawLine of lines) {
    ...
    if (h3) {
      closeList();
      html.push(`<h3>${inlineWeeklyMarkdown(h3[1])}</h3>`);
    } else if (h2) {
      closeList();
      html.push(`<h2>${inlineWeeklyMarkdown(h2[1])}</h2>`);
    } else if (h1) {
      closeList();
      html.push(`<h1>${inlineWeeklyMarkdown(h1[1])}</h1>`);
    } else if (bullet) {
      if (!inList) {
        html.push("<ul>");
        inList = true;
      }
      html.push(`<li>${inlineWeeklyMarkdown(bullet[1])}</li>`);
    } else if (!trimmed.startsWith("<!--")) {
      closeList();
      html.push(`<p>${inlineWeeklyMarkdown(trimmed)}</p>`);
    }
  }
...
function applyWeeklyHumanAnalysis() {
  ...
  if (!card) {
    ...
    card.className = "weekly-insight review-card";
  ...
  card.innerHTML = `
    <div class="weekly-human-head">
      <div>
        <p class="eyebrow">Weekly Intelligence · MD Analysis</p>
        <h3>종합분석</h3>
      </div>
      <span class="weekly-human-period">${escapeWeeklyHtml(weeklyHumanReport.label || weeklyHumanReport.period)}</span>
    </div>
    <div class="weekly-human-document">${weeklyHumanHtml}</div>
  `;
}
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initWeeklyHumanAnalysis, { once: true });
} else {
  initWeeklyHumanAnalysis();
}
